...
MongoManager.prototype = {
    ...
    deleteCard: function deleteCard(id, mySprint) {
        var companySprint = mySprint.split("/");
        this.mongoDb().collection('sprints').update({"name": companySprint[1], "company": companySprint[0]}, {$pull: {'tasks': {'id': id}}},function (err, result) {
            if (err) {
                return console.dir(err);
            }
        });
    ...
    findSprint: function findSprint(mySprint, callback) {
        var companySprint = mySprint.split("/");
        this.mongoDb().collection('sprints').findOne({company: companySprint[0], name: companySprint[1]}, function (err, item) {
            callback(item);
        });
    }
};
